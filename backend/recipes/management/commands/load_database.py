        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                ...
                if name and unit and name not in existing_names:
                    ingredients_to_add.append(Ingredients(name=name, measurement_unit=unit))
                    existing_names.add(name)

        if ingredients_to_add:
            Ingredients.objects.bulk_create(ingredients_to_add, batch_size=1000)
            self.stdout.write(
                self.style.SUCCESS(f'Загружено {len(ingredients_to_add)} ингредиентов')
            )
