...
class SubscriptionSerializer(UserProfileSerializer):
    ...
    def get_is_subscribed(self, obj):
        request = self.context.get('request')
        return request.user.is_authenticated and obj.subscribers.filter(user=request.user).exists()
...
class RecipeWriteSerializer(serializers.ModelSerializer):
    ...
    ingredients = IngredientCreateSerializer(many=True, write_only=True, required=True)
    ingredients_read = RecipeIngredientSerializer(many=True, source='recipe_ingredient', read_only=True)
    ...
    class Meta:
        model = Recipe
        fields = ('id', 'author', 'ingredients', 'ingredients_read', 'is_favorited',
    ...
    def get_is_favorited(self, obj):
        request = self.context.get('request')
        return request.user.is_authenticated and obj.favorited_by.filter(user=request.user).exists()

    def get_is_in_shopping_cart(self, obj):
        request = self.context.get('request')
        return request.user.is_authenticated and obj.in_cart.filter(user=request.user).exists()
